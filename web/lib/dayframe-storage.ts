...
export function decodeBackup(text: string): DayframeState {
  ...
  if (raw.schema === 5) {
    ...
    const validTask = (t: any) => t && typeof t.id === "string" && typeof t.title === "string" && (t.date === "" || /^\d{4}-\d{2}-\d{2}$/.test(t.date)) && Number.isFinite(t.duration) && t.duration >= 15 && typeof t.completed === "boolean";
    if (Object.entries(raw.plans).some(([date, tasks]) => !/^\d{4}-\d{2}-\d{2}$/.test(date) || !Array.isArray(tasks) || tasks.some(t => !validTask(t) || t.date !== date)) || raw.backlog.some((t: any) => !validTask(t))) throw new Error("Některé úkoly v záloze nejsou platné.");
    if (raw.routines.some((r: any) => !r || typeof r.id !== "string" || typeof r.title !== "string" || !Number.isFinite(r.duration) || r.duration < 15 || !["daily", "weekly", "alternate"].includes(r.frequency))) throw new Error("Rutiny v záloze nejsou platné.");
    if (raw.milestones.some((m: any) => !m || typeof m.id !== "string" || typeof m.title !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(m.date))) throw new Error("Milníky v záloze nejsou platné.");
